...
## v0.2.13 — V2 #11 模块重构（2026-06-05）

**TL;DR**：把 `server.py` 1124 行的"业务流水线"代码剥离到 4 个新模块 + 1 个新子包（共 18 个新文件，1642 行新代码）。server.py 瘦身到 664 行（-41%）。学官方 78/xiaozhi-esp32-server 的 handle/textHandler + ToolManager + per-session MCPClient + asyncio.Lock 设计。

### 新增模块

| 模块 | 行 | 职责 |
|---|---|---|
| `mcp/client.py` | 179 | MCPClient per-session + asyncio.Lock（V2 #7.7 race 完整修复蓝图）|
| `mcp/manager.py` | 287 | ToolType (StrEnum) + ToolExecutor (ABC) + ToolManager 派发器 |
| `mcp/handlers.py` | 257 | 5 个 server.py 方法的 thin shim → module-level functions |
| `handle/textMessageHandler.py` | 48 | 抽象基类 |
| `handle/textMessageHandlerRegistry.py` | 71 | 注册表（type → handler）|
| `handle/textMessageProcessor.py` | 86 | 派发器（替换 _main_loop match-case）|
| `handle/textHandler/abortMessageHandler.py` | 31 | abort 消息处理 |
| `handle/textHandler/helloMessageHandler.py` | 38 | 异常 hello 处理 |
| `handle/textHandler/listenMessageHandler.py` | 68 | listen 状态机 |
| `handle/textHandler/mcpMessageHandler.py` | 70 | MCP JSON-RPC 路由 |
| `pipeline/turn.py` | 226 | ASR → LLM tool-use 循环 → TTS 完整流水线 |
| `pipeline/tts.py` | 118 | TTS 串流协议 + ConnectionClosed 优雅处理 |
| `audio/handler.py` | 129 | VAD + Opus 解码 + 缓冲区 + wake-grace |

### server.py 瘦身

| 方法 | 拆分前 | 拆分后 |
|---|---|---|
| `_process_turn` | 43 行 | 3 行 shim → pipeline/turn.process_turn |
| `_process_text` | 158 行 | 3 行 shim → pipeline/turn.process_text |
| `_send_tts` | 86 行 | 3 行 shim → pipeline/tts.send_tts |
| `_handle_audio` | 66 行 | 3 行 shim → audio/handler.handle_audio |
| `_handle_listen` | 41 行 | shim → handle/textHandler/listenMessageHandler |
| `_handle_abort` | 9 行 | shim → handle/textHandler/abortMessageHandler |
| `_handle_mcp` | 53 行 | shim → handle/textHandler/mcpMessageHandler |
| `_send_mcp_call` | 32 行 | shim → mcp/handlers.send_mcp_call |
| `_register_device_tools` | 69 行 | shim → mcp/handlers.register_device_tools |
| `_cleanup_session_tools` | 26 行 | shim → mcp/handlers.cleanup_session_tools |
| `_build_llm_tools_payload` | 21 行 | shim → mcp/handlers.build_llm_tools_payload |
| `_dispatch_tool` | 30 行 | shim → mcp/handlers.dispatch_tool |
| `_end_wake_grace` | 13 行 | shim → audio/handler.end_wake_grace |
| `_cancel_wake_grace` | 22 行 | shim → audio/handler.cancel_wake_grace |
| **server.py 总** | **1124** | **664**（**-41%**）|

### 不变量（V2 #11.2 五条）

1. **协议 0 改** — xiaozhi WS 消息 schema、MCP JSON-RPC、xiaozhi 状态机 1 字节不改
2. **DB 0 改** — `bridge.db` schema 不加列、不改类型
3. **配置 0 改** — `config.yaml` schema 0 改
4. **API 0 改** — `bridge-api` HTTP 端点 0 改
5. **依赖 0 改** — `pyproject.toml` 0 改（只搬家，不引新依赖）

### 测试

- **新增**：`tests/test_v2_13_modules.py`（16 个新测试 — MCPClient 6 + ToolManager 5 + handlers 5）
- **总数**：147 passed + 7 skipped = 154
- **CI**：ruff ✅ mypy ✅ pytest ✅
...
- 78/xiaozhi-esp32-server `core/handle/textMessageHandlerRegistry.py`（47 行）
- 78/xiaozhi-esp32-server `core/handle/textMessageProcessor.py`（44 行）
- 78/xiaozhi-esp32-server `core/providers/tools/unified_tool_manager.py`（200+ 行）
- 78/xiaozhi-esp32-server `core/providers/tools/device_mcp/mcp_handler.py`（403 行 MCPClient）
- 本仓库 `docs/refactor-v2.13-modules.md`（30.7KB，12 节 + 2 附录的完整规划）
