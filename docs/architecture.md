...
>
> **V2 #12 现状（2026-06-05）**：V2 全部 12 个阶段完成。
> - V2 #1（v0.2.2）真 ASR：sherpa-onnx streaming-zipformer
> - V2 #2（v0.2.3-0.2.5）真 TTS：edge-tts 默认 + iptables 持久化
> - V2 #3（v0.2.0）FastAPI HTTP API
> - V2 #4（v0.2.1）设备关联
> - V2 #5（web 0.2.0）智控台接真数据
> - V2 #6（v0.2.6）多设备 + 元数据
> - V2 #6.1（v0.2.7）+ V2 #6.2（v0.2.8）+ V2 #6.3（v0.2.8）鉴权 + E2E 修复
> - **V2 #7（v0.2.11 + v0.2.12）反向 MCP**：bridge 主动调 esp32-side 工具（set_volume / set_brightness / get_device_status）
> - **V2 #8（v0.2.9）esp32 OTA + 服务端 Silero VAD + 链路全通**
> - **V2 #10（v0.2.10）SenseVoice ASR 替换 zipformer（长句 0 乱码）**
>
>
...
```
   ...
   │  流式 SSE 返回文本                                        │
   │                                                          │
   │  **V2 #7 改变**：bridge 现在会传 `tools[]` 给 openclaw，  │
   │  且 openclaw 会把 `tool_calls` SSE delta 原样传回。       │
   │  bridge 拿到 `tool_call` 事件后，调 MCP registry，        │
   │  对 esp32 端工具发 JSON-RPC `tools/call` 并 await 响应。  │
   │  响应以 `role=tool` message 重新喂回 openclaw，           │
   │  LLM 继续产生最终文本。                                   │
   └──────────────────────────────────────────────────────────┘
```
...
**V1 + V2 #7 方向**：bridge **既是 MCP server（向设备暴露设备能力查询 / 音量 / LED 控制）**，**也是 MCP client（向设备要 `tools/list`）**。
...
methods = {
    ...
    "tools/list": ...,    # 双向
    "tools/call": ...,    # 双向（V1：设备→bridge；V2 #7：bridge→设备）
}
```

**bridge 内置工具**（`mcp/tools.py`）：

- `FunctionTool`（V1 占位，可**在** bridge 本**地**运**行** Python 函**数**）：
  - V1 实际实现的 3 个，V2 增**加**更**多**（`get_time` / `get_weather` / `get_date` / 接 IoT / HA / 米家等）
- `DeviceToolHandler`（V2 #7，**bridge→esp32**）：
  - `get_device_status`（esp32 端 `self.get_device_status`）— 返回音量 / 亮度 / Wi-Fi / 电池
  - `set_volume`（esp32 端 `self.audio_speaker.set_volume`）— 0-100
  - `set_brightness`（esp32 端 `self.screen.set_brightness`）— 0-100

**V2 #7 跟 V1 的关键变化**：
- V1 工具是 "**log only**"（**只**打**日**志**）**，**V2 #7 工具是 "**forward to esp32**"**（**经** JSON-RPC 透**过** xiaozhi WS 发**到** esp32 执**行**）**
- V2 #7 加**了** `ESP32_NAME_MAP` 桥**接**端**名**字** → esp32 端**名**字**（**前**向**兼**容**）**
- V2 #7 加**了** session-scoped 追**踪** `_session_tool_owners`，会**话**关**闭**时** unregister 避**免** race condition

**跟 openclaw tool 的关系**：
- V1 阶**段**：openclaw agent **自己有** 一套 tool registry（web_search / IoT 平台接入 / 知识库等），bridge **不**接**入**。bridge **不**传** `tools[]` 给 openclaw，openclaw **自**己** 调**用**（bridge 看**不**到** tool_calls）**
- **V2 #7 阶**段**：bridge **现**在**会**传** `tools[]` 给 openclaw（包**括** esp32 端** DeviceToolHandler），openclaw 把 `tool_calls` SSE delta 原**样**回**传**给 bridge。bridge 拿**到** `tool_call` 事**件**后调 MCP registry，对 esp32 端**工**具**发** JSON-RPC `tools/call` 并 await 响**应**。**响**应**以** `role=tool` message 重**新**喂**回** openclaw，LLM 继**续**产**生**最**终**文**本**。**详**细**实**现**见** `mcp/tools.py` `DeviceToolHandler` + `llm/openclaw.py` tool_acc 累**加**器 + `server.py` `_process_text` tool-use loop**
