...
**V1 方向**：bridge **既是 MCP server（向设备暴露 `get_time` / `get_weather` / demo 灯控制）**，**也是 MCP client（向设备要 `tools/list`）**。
...
**bridge 内置工具**（`mcp/tools.py`，190 行）：

- `get_time` — 返回当前 UTC + 本地时区时间
- `get_weather` — 返回 mock 天气（V1 没接真 API）
- `turn_on_light` / `turn_off_light` — 模拟灯控制（**演示用**，不接真 IoT）

这些工具是 **bridge 实现** 的，**不走** openclaw tool calling。
...
```
硬件 → bridge: {"type": "mcp", "payload": {"jsonrpc": "2.0", "method": "tools/call", "params": {"name": "turn_on_light", "arguments": {"room": "living_room"}}}}
bridge: 解析 JSON-RPC → 调 mcp/tools.py 里 turn_on_light(...)
         (V1 是 mock，**不打** 真灯，只返回成功)
